...
export function parseLLMIgnore(filePath: string): LLMIgnoreConfig {
  const content = readFileSync(filePath, 'utf-8');
  const lines = content.split('\n').map(line => line.trim());
  console.log(process.cwd());
  const config: LLMIgnoreConfig = {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith('starting_directory:')) {
      ...
      if (key && value) config.project_info[key] = value;
...
export function matchesPattern(path: string, fullPath: string, patterns: string[], baseDir: string, isExclude: boolean = false): boolean {
...
export function detectLanguage(dir: string): string | null {
  const files = readdirSync(dir);
  if (files.includes('package.json')) return 'node';
  if (files.includes('requirements.txt') || files.includes('pyproject.toml')) return 'python';
  if (files.includes('go.mod')) return 'go';
  if (files.includes('CMakeLists.txt') || files.some(f => f.endsWith('.cpp'))) return 'cpp';
...
export async function createFromTemplate(language?: string): Promise<void> {
  const templates = listTemplates();
  let selectedLanguage = language;

  if (!selectedLanguage) {
    const detected = detectLanguage(process.cwd());
    const response = await enquirer.prompt<{ language: string }>({
      type: 'select',
      name: 'language',
