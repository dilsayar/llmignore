...
export function countFiles(dir, config, baseDir, maxSize) {
    let count = 0;
    try {
...
export function buildFileTree(dir, config, baseDir, progressBar, logs, maxSize) {
    let tree = [];
    let contents = [];
    let stats = { files: 0, lines: 0, tokens: 0, bytes: 0 };
    let files;
    try {
    ...
    for (const file of files) {
        ...
        if (statsFile.isDirectory()) {
            ...
            try {
                ...
                contents.push(`=== BEGIN FILE: ${relPath} ===`, `Path: ${fullPath}`, content, `=== END FILE: ${relPath} ===\n`);
            }
            catch (error) {
